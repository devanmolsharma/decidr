...
Same `decide()` call as every other backend. `api_key` can come from `ANTHROPIC_API_KEY` instead.
...
`LiteLLMBackend`'s response normalization is unit-tested against objects matching LiteLLM's documented `ChatCompletion` shape, and its request construction is verified the same way. What is **not** verified in this project: an actual live call to OpenAI, Anthropic, or any other hosted provider through it — that needs an API key this project doesn't have. The examples above are correct usage, not confirmed wire behavior. If a specific provider does something LiteLLM's documented shape doesn't predict, that's the likely place to look first.
